...
{
   unsigned char ctrlHum, ctrlMeas, config;
...
)
{
   ...
   ctrlHum = (unsigned char)m_settings.humOSR;
   ...
   ctrlMeas = ((unsigned char)m_settings.tempOSR << 5) | ((unsigned char)m_settings.presOSR << 2) | (unsigned char)m_settings.mode;
   // config register. (config[7:5] = standby time, config[4:2] = filter, ctrl_meas[0] = spi enable.)
   config = ((unsigned char)m_settings.standbyTime << 5) | ((unsigned char)m_settings.filter << 2) | (unsigned char)m_settings.spiEnable;
}
...
{
   bool success;
   unsigned char buffer[SENSOR_DATA_LENGTH];
...
)
{
   // Code based on calibration algorthim provided by Bosch.
   int32_t var1;
   unsigned char   dig_H1 =   m_dig[24];
   int16_t dig_H2 = (m_dig[26] << 8) | m_dig[25];
   unsigned char   dig_H3 =   m_dig[27];
